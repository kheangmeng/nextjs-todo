...
export default function BlogDetail ({ post, onGoHome, onSelectPost }: BlogDetailProps) {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [post]);
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 md:p-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {relatedPosts.map(relatedPost => (
                <RelatedPostCard key={relatedPost.id} post={relatedPost} onSelectPost={onSelectPost} />
              ))}
            </div>
          </div>
        )}
...
const RelatedPostCard = ({ post, onSelectPost }: RelatedPostCardProps) => (
  <div className="bg-slate-50 rounded-lg p-4 flex items-start space-x-4 hover:bg-slate-100 transition-colors cursor-pointer" onClick={() => onSelectPost(post)}>
